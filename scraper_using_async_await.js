...
const request = req(`${baseURL}shirts.php`, function (error, response, body) {
  if (error !== null) {
    ...
    async function f1() {
      const populateCSVArrayPromise = await populateCSVArray();
      console.log(populateCSVArrayPromise);
      console.log("This should be last!");
    };

    async function populateCSVArray() {
      return new Promise(async function (resolve, reject) {

        const requestLoopPromise = await requestLoop();
        console.log(requestLoopPromise);
        await createCSV();
        resolve("Successfully resolved populateCSVArray()");
        reject("Failed to resolve populateCSVArray()");
        ...
        function requestLoop() {
          return new Promise(function (resolve, reject) {
            req('http://shirts4mike.com/shirt.php?id=101', function (error, response, body) {
              ...
              dataForCSV.push([[`${title}`], [`${price}`], [`${time}`]]);
              console.log(price); // This should be the first thing to log to the console
              resolve("Successfully resolved requestLoopPromise()");
              reject("Failed to resolve requestLoopPromise()")
            });
          });
        };
